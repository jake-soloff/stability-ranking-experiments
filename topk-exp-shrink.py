...
# run experiments
def run_exp_shrink(eps, Klist, fulldf, n, N_rep):
    unique_user_ids = fulldf['user_id'].unique()
    ...
    for t in tqdm(range(N_rep)):
        ...
        data = fulldf[fulldf['user_id'].isin(sampled_user_ids)]
        ...
        all_mean_pd = global_sum / (global_count + 1) # apply shrinkage
        ...
        for i in range(n):
            ...
            leave_out_sum = global_sum - user_sum
            leave_out_count = global_count - user_count
            loo_mean_pd = leave_out_sum / (leave_out_count + 1) # apply shrinkage
            loo_mean = loo_mean_pd.to_numpy() # LOO average rating (unsorted)
    ...
    filename_all = f"topk_results_eps_{eps}_n_{n}_shrink.npz"
    ...
    filename_sum = f"topk_summary_eps_{eps}_n_{n}_shrink.csv"
...
delta_j, result_summary = run_exp_shrink(eps, Klist, Fulldf, n, N_rep)
...
fullResults = np.load('topk_results_eps_0.01_n_1000_shrink.npz')
...
for k_ind in range(len(Klist)):
    ...
    plt.title(fr"Stability comparison with $k={k}$",fontsize=20)
    plt.tight_layout()
